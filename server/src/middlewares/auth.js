...
const { sessionMatches } = require('../services/session');
...
const createAuth = ({ db = prisma } = {}) => async (req, res, next) => {
  const header = req.headers.authorization || '';
  const token = header.startsWith('Bearer ') ? header.slice(7) : null;
  if (!token) return fail(res, 401, '未登录');
  try {
    req.user = jwt.verify(token, process.env.JWT_SECRET, { algorithms: ['HS256'] });
    if (!Number.isInteger(req.user.userId)) throw new Error('Invalid user');
  } catch {
  ...
  try {
    const u = await db.user.findUnique({
      ...
      select: { status: true, storeId: true, role: true, sessionVersion: true },
    });
    if (!u || u.status !== 1) return fail(res, 401, '账号已注销或被停用');
    if (!sessionMatches(req.user, u)) return fail(res, 401, '登录已失效，请重新登录');
    req.user.sessionVersion = u.sessionVersion;
...
module.exports = { auth, adminOnly, createAuth };
