...
const file = useIsolatedDb(`manual-sale-processes-${process.pid}`);
...
const workerCode = `
const prisma = require('./src/config/prisma');
const { confirmSale } = require('./src/services/saleConfirmation');
process.once('message', async ({ input, actorId }) => {
  try {
    const result = await prisma.runWithTenant(1, () => confirmSale(input, actorId));
...
test('跨进程同确认ID并发与重启重试只生成一单、一笔收款和一次库存扣减', { timeout: 60000 }, async t => {
  const input = { requestId: 'multiprocess-manual-sale', items: [{ skuId: sku.id, quantity: 2, unitPrice: 10 }] };
  ...
  assert.equal(replay.replayed, true); assert.equal(replay.order.id, results[0].order.id);
